...
_THEME_TOKENS: dict[str, str] = {
    ...
    "radius": "1rem",
    "text": "#2f3a4b",
    "font_body": "'Open Sans', Arial, sans-serif",
...
DEFAULT_TEMPLATES: dict[str, dict] = {
    ...
    "contact": {
        ...
        "html": (
            ...
            + '<div class="web-contact-card"><div class="web-card-icon"><i class="fa-solid fa-circle-info"></i></div><h3>Kontakt</h3><p>Doplňte aktuální kontaktní údaje oddílu.</p></div>'
            ...
            + '<p style="margin-top:1.25rem"><a class="web-button" href="/kontakt">Kontaktní informace</a></p>'
            + '</div></div></section>'
        ),
        "css": "",
    },
...
# Keep the final ScoutComp 2.0 declarations theme-token aware.  This override
# intentionally comes last because the compatibility stylesheet above contains
# older bundled declarations for the same sc2 variables.
THEME_CSS += """
:root{--web-primary:var(--sc-primary,#0a224e);--web-accent:var(--sc-accent,#1e3a6e);--web-primary-2:var(--sc-primary-2,#2f3a4b);--web-bg:var(--sc-bg,#fff);--web-text:var(--sc-text,#2f3a4b);--web-muted:var(--sc-muted,#6b7280);--web-border:var(--sc-border,#e9e0d2);--web-cream:var(--sc-cream,#e1d3c1);--web-cream-2:var(--sc-cream-2,#f6ebd8);--web-radius:var(--sc-radius,1rem);--sc2-blue:var(--sc-primary,#003873);--sc2-navy:var(--sc-primary-2,#0a224e);--sc2-sky:var(--sc-accent,#00ade3);--sc2-green:var(--sc-secondary,#97c93e);--sc2-cream:var(--sc-cream,#e1d3c1);--sc2-paper:var(--sc-bg,#fffdf8);--sc2-ink:var(--sc-text,#2f3a4b);--sc2-font-body:var(--sc-font-body,'Open Sans',Arial,sans-serif);--sc2-font-heading:var(--sc-font-heading,Georgia,'Times New Roman',serif);--sc2-body:var(--sc-font-body,'Open Sans',Arial,sans-serif);--sc2-head:var(--sc-font-heading,Georgia,'Times New Roman',serif)}
.sc2-brand{display:inline-flex;align-items:center;gap:.65rem}.sc2-brand-logo{display:block;width:auto;height:2.75rem;max-width:9rem;object-fit:contain}.sc2-brand-logo:not([src]),.sc2-brand-logo[src=""],.sc2-hero-media img:not([src]),.sc2-hero-media img[src=""],.sc2-team-card img:not([src]),.sc2-team-card img[src=""],.sc2-card-image:not([src]),.sc2-card-image[src=""],.sc2-person img:not([src]),.sc2-person img[src=""]{display:none}.sc2-hero-media{background:radial-gradient(circle at 22% 30%,color-mix(in srgb,var(--sc2-sky) 32%,transparent),transparent 42%),linear-gradient(135deg,var(--sc2-navy),var(--sc2-blue))}.sc2-team-card{background:linear-gradient(145deg,var(--sc2-blue),var(--sc2-navy))}
"""
