...
const toolbarButtons = [
  ...
  "insertLink", "mediaLibrary", "embed", "symbols", "detailsBlock", "infoBox", "table", "hr", "|", "source", "fullsize", "preview", "print", "search",
];
...
export default function RichTextEditor({ value, onChange, disabled = false, height = 360, placeholder, className = "" }) {
  ...
  useEffect(() => {
    ...
    const initialize = async () => {
      ...
      editor = Jodit.make(hostRef.current, {
        ...
        controls: {
          mediaLibrary: { icon: "image", tooltip: "Vybrat z médií", exec: (instance) => { instance.s.save(); setMediaPickerOpen(true); return false; } },
  ...
  return <div className={`rich-text-editor ${className}`.trim()}>
    <div ref={hostRef} />
    {mediaPickerOpen && <MediaPickerModal title="Vybrat médium" onSelect={selectMedia} onClose={() => setMediaPickerOpen(false)} />}
    {dialog && <EditorDialog dialog={dialog} editor={editorRef.current} onClose={() => setDialog(null)} onInsert={insertDialogContent} />}
  </div>;
}

function EditorDialog({ dialog, editor, onClose, onInsert }) {
  const [value, setValue] = useState(dialog.type === "color" ? "#1f4f37" : "");
  const [linkText, setLinkText] = useState("");
  const title = { color: "Barva textu", link: "Vložit odkaz", embed: "Video nebo iframe", details: "Rozbalovací sekce", info: "Informační box" }[dialog.type];
  const submit = (event) => {
  ...
  return createPortal(<div className="rich-text-dialog-backdrop" role="dialog" aria-modal="true" aria-label={title} onClick={(event) => event.stopPropagation()}>
    <form className="rich-text-dialog" onSubmit={submit} onClick={(event) => event.stopPropagation()}>
      ...
      {dialog.type === "color" ? <input className="form-control form-control-color" type="color" value={value} onChange={(event) => setValue(event.target.value)} /> : <>
        <label>{dialog.type === "link" ? "Adresa odkazu" : dialog.type === "embed" ? "URL YouTube/Vimeo nebo iframe kód" : dialog.type === "details" ? "Nadpis sekce" : "Nadpis boxu"}<input className="form-control mt-1" autoFocus type={dialog.type === "link" ? "url" : "text"} value={value} onChange={(event) => setValue(event.target.value)} /></label>
...
EditorDialog.propTypes = { dialog: PropTypes.object.isRequired, editor: PropTypes.object, onClose: PropTypes.func.isRequired, onInsert: PropTypes.func.isRequired };
