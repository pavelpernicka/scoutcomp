...
export default function RevisionsDialog({ pageId, onClose, onRestored }) {
  const { t } = useTranslation();
  const queryClient = useQueryClient();
  const revisionsQuery = useQuery({ queryKey: ["web", "revisions", pageId], queryFn: () => cmsApi.listRevisions(pageId) });
  const restore = useMutation({
    mutationFn: (revisionId) => cmsApi.restoreRevision(pageId, revisionId),
    onSuccess: async () => {
      if (onRestored) await onRestored();
      else await queryClient.invalidateQueries({ queryKey: ["web", "page", pageId] });
      onClose();
    },
  });
...
RevisionsDialog.propTypes = { pageId: PropTypes.number.isRequired, onClose: PropTypes.func.isRequired, onRestored: PropTypes.func };
