...
export default function ThemesPage() {
  ...
  return <section>
    ...
    {themesQuery.isLoading ? <LoadingSpinner /> : themes.length === 0 ? <div className="web-admin-empty"><i className="fas fa-palette" /><h3>{t("web.empty.themesTitle")}</h3><p>{t("web.empty.themesBody")}</p></div> : <div className="web-theme-grid">{themes.map((theme) => { const versionPreview = (theme.versions || []).find((v) => v.preview_url); const previewUrl = theme.preview_url || versionPreview?.preview_url; const versionId = theme.versions?.[0]?.id || theme.id; const activeTheme = (theme.versions || []).some((v) => v.active); return <article key={theme.id || theme.theme_id} className={`web-theme-card ${activeTheme ? "active" : ""}`}>{previewUrl ? <MediaPreview src={previewUrl} alt="" /> : <div className="web-theme-placeholder"><i className="fas fa-window-maximize" /></div>}<div className="web-theme-body"><div><h3>{theme.name}</h3><p>{theme.description || t("web.noDescription")}</p></div><dl><div><dt>{t("web.version")}</dt><dd>{theme.versions?.[0]?.version || "—"}</dd></div><div><dt>{t("web.author")}</dt><dd>{theme.author || "—"}</dd></div></dl><div className="web-theme-actions">{activeTheme ? <><span className="web-status published"><i className="fas fa-check" />{t("web.states.active")}</span><button type="button" className="btn btn-sm btn-outline-primary" onClick={() => navigate("/admin/web/design/templates")}><i className="fas fa-pen me-1" />{t("web.editTheme")}</button></> : <button type="button" className="btn btn-sm btn-primary" disabled={activate.isPending} onClick={() => activate.mutate(versionId)}>{t("web.commands.activate")}</button>}<button type="button" className="btn btn-sm btn-outline-secondary" title={t("web.downloadTheme")} onClick={() => download.mutate({ id: versionId, name: theme.name })}><i className="fas fa-download" /></button><button type="button" className="btn btn-sm btn-outline-secondary" title={t("web.duplicateTheme")} onClick={() => { setDuplicating(theme.id); setDuplicateName(`${theme.name} (kopie)`); }}><i className="fas fa-copy" /></button><button type="button" className="btn btn-sm btn-outline-danger" disabled={activeTheme || uninstall.isPending} onClick={() => { if (window.confirm(t("web.confirmUninstallTheme"))) uninstall.mutate(versionId); }}>{t("web.commands.uninstall")}</button></div></div></article>; })}</div>}
