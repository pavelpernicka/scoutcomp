  ...
  return (
    <li className="inventory-tree-node" style={{ "--inventory-tree-depth": depth }}>
      <div className={`inventory-tree-row ${selectedPath === node.path ? "is-selected" : ""}`}>
        {hasChildren ? <button type="button" className="inventory-tree-toggle" onClick={() => onToggle(node.id)} aria-label={expanded ? t("inventory.collapseNode", { name: node.name }) : t("inventory.expandNode", { name: node.name })} aria-expanded={expanded}><i className={`fas fa-chevron-${expanded ? "down" : "right"}`} /></button> : <span className="inventory-tree-toggle-spacer" />}
        ...
        {editable ? <div className="inventory-tree-actions">
          <button type="button" className="btn btn-sm btn-link" title={t("inventory.addChild")} onClick={() => onCreateChild(node)}><i className="fas fa-plus" /></button>
          <button type="button" className="btn btn-sm btn-link" title={t("common.edit")} onClick={() => onEdit(node)}><i className="fas fa-pen" /></button>
          <button type="button" className="btn btn-sm btn-link text-danger" title={t("common.delete")} onClick={() => onDelete(node)}><i className="fas fa-trash" /></button>
        </div> : null}
      </div>
